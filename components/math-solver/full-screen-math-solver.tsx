...
//     };
//   }, [brushColor, brushSize, isEraser, showSolution]); // Add showSolution as dependency
...
//   // Log latexExpressions to console
//   useEffect(() => {
//     console.log("Debug latexExpressions:", JSON.stringify(latexExpressions, null, 2));
//   }, [latexExpressions]);

//   // Log solution panel state
//   useEffect(() => {
//     console.log("Solution Panel State:", { solution, showSolution, equation, steps });
//   }, [solution, showSolution, equation, steps]);
...
export function FullScreenMathSolver({
  ...
  const [showSolution, setShowSolution] = useState(false);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const [problemType, setProblemType] = useState<"Math" | "Physics" | "Abstract" | null>(null);
  const [selectedProblemType, setSelectedProblemType] = useState<"Math" | "Physics" | "Abstract">("Math");

  const canvasRef = useRef<HTMLCanvasElement>(null);
  const canvasContextRef = useRef<CanvasRenderingContext2D | null>(null);
  const [isDrawing, setIsDrawing] = useState(false);
  const [brushColor, setBrushColor] = useState("#000000");
  const [brushSize, setBrushSize] = useState(10); // Increased from 5 to 10
  const [isEraser, setIsEraser] = useState(false);
  ...
  useEffect(() => {
    ...
    // Restore last canvas state if available
    if (canvasHistory.length > 0 && historyIndex >= 0) {
      const { imageData, latexExpressions: historyLatex } = canvasHistory[historyIndex];
      ctx.putImageData(imageData, 0, 0);
      setLatexExpressions(historyLatex);
    } else {
      saveCanvasState();
    }
  ...
  }, [brushColor, brushSize, isEraser, showSolution]);
  ...
  // Log solution panel state
  useEffect(() => {
    console.log("Solution Panel State:", { solution, showSolution, equation, steps, problemType });
  }, [solution, showSolution, equation, steps, problemType]);
  ...
  const saveCanvasState = () => {
    ...
    try {
      ...
      setCanvasHistory((prev) => [...prev, { imageData, latexExpressions }]);
      setHistoryIndex((prev) => prev + 1);
    } catch (error) {
      console.error("Error saving canvas state:", error);
    }
  ...
  const undo = () => {
    ...
    const { imageData, latexExpressions: historyLatex } = canvasHistory[newIndex];
    ctx.putImageData(imageData, 0, 0);
    setLatexExpressions(historyLatex);
  };
  ...
  const solveEquation = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8900/calculate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ image: dataURL, dict_of_vars: {}, problem_type: selectedProblemType }),
      });
      ...
      // Check for errors in the steps array
      const hasError = steps.some((step: string) => step.toLowerCase().includes("error"));
      if (hasError) {
        const errorStep = steps.find((step: string) => step.toLowerCase().includes("error")) || "Unknown error occurred.";
        throw new Error(errorStep);
      }

      // Determine the problem type based on the response
      let detectedProblemType: "Math" | "Physics" | "Abstract" | null = null;
      if (steps.some((step: string) => step.toLowerCase().includes("fbd") || step.toLowerCase().includes("newton") || step.toLowerCase().includes("tension"))) {
        detectedProblemType = "Physics";
      } else if (steps.some((step: string) => step.toLowerCase().includes("abstract concept"))) {
        detectedProblemType = "Abstract";
      } else {
        detectedProblemType = "Math";
      }
      setProblemType(detectedProblemType);
      ...
      if (equation && solution && steps.some((step: string) => step.includes("Assign"))) {
        ...
        // Case 1, 4, 5, or 6: Expression, Graphical Math, Physics, or Abstract
        latex = `${equation} = ${solution}`;
      } else if (equation && equation !== "Unrecognized drawing" && equation !== "Error") {
        // Partial result
        latex = `${equation}`;
      ...
      if (latex.trim() && equation !== "Unrecognized drawing" && equation !== "Error") {
        ...
        console.warn("Skipping invalid or empty LaTeX:", { equation, solution, steps });
        if (equation === "Unrecognized drawing") {
          setErrorMessage("Unable to recognize the drawing. Please ensure the expression is clear and legible.");
        }
      }
    } catch (error) {
      console.error("Error solving equation:", error);
      let errorMsg = (error as Error).message;
      if (errorMsg.toLowerCase().includes("unrecognized drawing")) {
        errorMsg += " Please ensure the expression is clear and legible.";
      }
    ...
    <div className="fixed inset-0 bg-background z-50 flex flex-col">
      <div className="h-[60px] border-b flex justify-between items-center px-4 bg-card">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold">Math & Physics Solver</h1>
          <select
            value={selectedProblemType}
            onChange={(e) => setSelectedProblemType(e.target.value as "Math" | "Physics" | "Abstract")}
      ...
      <div className="flex-1 relative" style={{ height: "calc(100vh - 60px)" }}>
        <div className="absolute inset-0 w-full h-full" style={{ position: "relative", zIndex: 1 }}>
          ...
          {errorMessage && (
            <div className="absolute top-20 left-1/2 transform -translate-x-1/2 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded z-50 max-w-[80%]">
              <span>{errorMessage}</span>
            </div>
          )}
              ...
              <div className="space-y-6">
                <div className="bg-card p-4 rounded-lg border">
                  <h2 className="text-xl font-semibold mb-2">
                    {problemType === "Physics" ? "Physics Problem" : problemType === "Abstract" ? "Abstract Concept" : "Equation/Problem"}
                  </h2>
                  <p className="text-lg font-mono">{equation || "No equation"}</p>
                </div>
                <div className="bg-card p-4 rounded-lg border">
